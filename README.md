...
N/A 不是漏测，是语义/静态不可锚定：¹ 纯范围聚合只有范围引用，"引用错位"语义并入范围漏算；² 公式内无运算符；³ COUNT 对全数值行计数恒等于格数（值中性，会被按"值正确"驳回）；⁴ 四则公式无范围引用；⁵ 由 SUMIF 的 sum_range 错列一格表达；⁶ criteria 运算符在字符串常量内，静态信号结构上不可见且行尾聚合格豁免兜底（探针实测候选集为空）。
